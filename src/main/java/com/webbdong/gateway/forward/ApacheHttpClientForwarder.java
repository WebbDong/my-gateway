    ...
    public FullHttpResponse forward(String forwardUrl, FullHttpRequest fullRequest) {
        ...
        try (CloseableHttpResponse response = client.execute(httpGet)) {
            if (HttpStatus.SC_OK != response.getStatusLine().getStatusCode()) {
                return FullHttpResponseUtil.createResponseByStatusCode(response.getStatusLine().getStatusCode());
            }
            FullHttpResponse httpResponse = new DefaultFullHttpResponse(
                    HttpVersion.HTTP_1_1, HttpResponseStatus.OK,
                    Unpooled.wrappedBuffer(EntityUtils.toByteArray(response.getEntity())));
            Arrays.stream(response.getAllHeaders()).forEach(h -> httpResponse.headers().set(h.getName(), h.getValue()));
            return httpResponse;
        } catch (IOException e) {
            e.printStackTrace();
        }
